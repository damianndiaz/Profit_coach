"""
Sistema de Entrenamiento Híbrido ProFit Coach
Metodología que combina ejercicios tradicionales con innovaciones funcionales
Principios de variabilidad y creatividad para rutinas únicas
"""

# ================================
# 🎯 PRINCIPIOS DE ENTRENAMIENTO HÍBRIDO
# ================================

TRAINING_PRINCIPLES = {
    "variabilidad": {
        "concepto": "Nunca repetir rutinas idénticas - siempre variar algo",
        "estrategias": [
            "Cambiar planos de movimiento",
            "Modificar implementos",
            "Alterar rangos de repeticiones",
            "Variar tiempos de descanso",
            "Combinar patrones de movimiento"
        ]
    },
    
    "hibridacion": {
        "concepto": "Combinar ejercicios tradicionales con innovaciones funcionales",
        "enfoque": [
            "Ejercicios tradicionales como base sólida",
            "Variaciones funcionales para transferencia",
            "Movimientos específicos del deporte",
            "Elementos creativos para motivación",
            "Progresiones naturales entre métodos"
        ]
    },
    
    "especificidad": {
        "concepto": "Adaptar según deporte, nivel y objetivos específicos",
        "factores": [
            "Demandas energéticas del deporte",
            "Patrones de movimiento característicos", 
            "Nivel de experiencia del atleta",
            "Equipamiento disponible",
            "Fase de la temporada"
        ]
    }
}

# ================================
# 🏗️ ESTRUCTURA METODOLÓGICA PROFESIONAL
# ================================

ROUTINE_METHODOLOGY = {
    "bloque_1": {
        "nombre": "Activación Glútea",
        "enfoque": "Activación específica del complejo glúteo y estabilizadores",
        "principios": [
            "Activación selectiva de glúteo medio y mayor",
            "Estabilización de pelvis y caderas",
            "Preparación para movimientos complejos",
            "Corrección de inhibiciones musculares"
        ],
        "categorias": ["activacion_glutea", "estabilizacion_pelvica", "movilidad_cadera"]
    },
    
    "bloque_2": {
        "nombre": "Dinámico/Potencia/Diagonales/Zona Media",
        "enfoque": "Adaptable según objetivo del deportista - potencia, movimientos diagonales o zona media",
        "principios": [
            "Selección según necesidades específicas del atleta",
            "Movimientos multiplanares y funcionales",
            "Transferencia directa al gesto deportivo",
            "Progresión en complejidad y velocidad"
        ],
        "categorias": ["potencia_explosiva", "movimientos_diagonales", "zona_media_dinamica", "coordinacion"]
    },
    
    "bloque_3": {
        "nombre": "Fuerza 1",
        "enfoque": "Primer bloque de fuerza funcional - patrones fundamentales",
        "principios": [
            "Patrones básicos de empuje y tracción",
            "Movimientos unilaterales y bilaterales",
            "Combinación tradicional-funcional",
            "Base sólida para progresiones"
        ],
        "categorias": ["fuerza_fundamental", "patrones_basicos", "unilaterales"]
    },
    
    "bloque_4": {
        "nombre": "Fuerza 2", 
        "enfoque": "Segundo bloque de fuerza funcional - patrones complejos y combinados",
        "principios": [
            "Movimientos combinados multiarticulares",
            "Patrones asimétricos y rotacionales",
            "Mayor complejidad coordinativa",
            "Transferencia específica al deporte"
        ],
        "categorias": ["fuerza_compleja", "movimientos_combinados", "transferencia_deportiva"]
    },
    
    "bloque_5": {
        "nombre": "Contraste/Preventivos/RSA",
        "enfoque": "Trabajo de contraste, prevención de lesiones y acondicionamiento específico",
        "principios": [
            "Contraste entre fuerza y velocidad",
            "Ejercicios preventivos específicos",
            "RSA (Repeated Sprint Ability) según deporte",
            "Optimización de la recuperación"
        ],
        "categorias": ["contraste_velocidad", "prevencion_lesiones", "RSA_especifico", "recuperacion"]
    },
    
    "alternativa_circuito": {
        "nombre": "Circuito Integral de 6 Ejercicios",
        "enfoque": "Reemplaza Fuerza 1 y Fuerza 2 por un circuito que integra todas las capacidades",
        "formato": "6 ejercicios x 5 series",
        "principios": [
            "Integración de fuerza, potencia, velocidad y zona media",
            "Variabilidad constante en ejercicios",
            "Eficiencia temporal máxima",
            "Estímulo completo en un solo bloque"
        ],
        "categorias": ["circuito_integral", "capacidades_mixtas", "eficiencia_temporal"]
    }
}

# ================================
# 🎨 CATEGORÍAS DE EJERCICIOS (SIN LISTAS FIJAS)
# ================================

EXERCISE_CATEGORIES = {
    "patrones_fundamentales": {
        "descripcion": "Movimientos básicos que todo atleta debe dominar",
        "tipos": ["empuje", "traccion", "sentadilla", "bisagra_cadera", "locomocion", "rotacion"],
        "variabilidad": "Desde versiones básicas hasta progresiones complejas"
    },
    
    "movimientos_funcionales": {
        "descripcion": "Ejercicios que imitan patrones naturales de movimiento",
        "tipos": ["animales", "combos_multiplanares", "unilaterales", "asimetricos"],
        "variabilidad": "Infinitas combinaciones y progresiones posibles"
    },
    
    "especificos_deporte": {
        "descripcion": "Movimientos directamente relacionados con el deporte",
        "tipos": ["gestos_tecnicos", "situaciones_juego", "capacidades_especificas"],
        "variabilidad": "Completamente personalizable según el deporte"
    },
    
    "metodos_avanzados": {
        "descripcion": "Técnicas de entrenamiento especializadas",
        "tipos": ["pliometria", "contrastes", "complejos", "circuitos", "intervalos"],
        "variabilidad": "Parámetros modificables infinitamente"
    }
}

# ================================
# 🏃‍♂️ ADAPTACIONES POR DEPORTE (PRINCIPIOS)
# ================================

SPORT_ADAPTATION_PRINCIPLES = {
    "deportes_intermitentes": {
        "ejemplos": ["futbol", "rugby", "basquet", "hockey"],
        "enfoque": [
            "Capacidad de repetir esfuerzos de alta intensidad",
            "Cambios de dirección explosivos",
            "Toma de decisiones rápidas",
            "Resistencia a la velocidad específica"
        ]
    },
    
    "deportes_ciclicos": {
        "ejemplos": ["natacion", "ciclismo", "remo", "atletismo_fondo"],
        "enfoque": [
            "Eficiencia técnica bajo fatiga",
            "Desarrollo de sistemas energéticos específicos",
            "Economía de movimiento",
            "Periodización de cargas"
        ]
    },
    
    "deportes_precision": {
        "ejemplos": ["tenis", "golf", "tiro", "arqueria"],
        "enfoque": [
            "Control motor fino",
            "Estabilidad bajo presión",
            "Concentración prolongada",
            "Precisión técnica"
        ]
    },
    
    "deportes_combate": {
        "ejemplos": ["boxeo", "artes_marciales", "lucha"],
        "enfoque": [
            "Potencia explosiva multidireccional",
            "Resistencia anaeróbica",
            "Agilidad reactiva",
            "Adaptabilidad táctica"
        ]
    }
}

# ================================
# 📊 GUÍAS DE PROGRESIÓN POR NIVEL
# ================================

PROGRESSION_GUIDELINES = {
    "principiante": {
        "filosofia": "Construcción de base sólida con énfasis en técnica",
        "metodos_preferidos": ["ejercicios_tradicionales_basicos", "progresiones_controladas"],
        "innovacion": "Introducir gradualmente elementos funcionales simples",
        "parametros": {"volumen": "bajo-moderado", "intensidad": "controlada", "complejidad": "simple"}
    },
    
    "intermedio": {
        "filosofia": "Desarrollo de capacidades con mayor variabilidad",
        "metodos_preferidos": ["hibrido_tradicional_funcional", "progresiones_moderadas"],
        "innovacion": "Combinar sistemáticamente tradicional con funcional",
        "parametros": {"volumen": "moderado", "intensidad": "variable", "complejidad": "moderada"}
    },
    
    "avanzado": {
        "filosofia": "Optimización del rendimiento con especificidad alta",
        "metodos_preferidos": ["metodos_avanzados", "especificidad_alta"],
        "innovacion": "Crear combinaciones únicas según necesidades",
        "parametros": {"volumen": "alto", "intensidad": "alta", "complejidad": "compleja"}
    },
    
    "elite": {
        "filosofia": "Individualización total y periodización sofisticada",
        "metodos_preferidos": ["completamente_personalizado", "innovacion_constante"],
        "innovacion": "Desarrollo de métodos únicos para el atleta",
        "parametros": {"volumen": "variable", "intensidad": "máxima", "complejidad": "muy_compleja"}
    }
}

# ================================
# 🎯 PRINCIPIOS DE CREATIVIDAD INTELIGENTE
# ================================

CREATIVITY_PRINCIPLES = {
    "variacion_sistematica": {
        "concepto": "Cambiar elementos manteniendo objetivos",
        "estrategias": [
            "Modificar un solo elemento por sesión",
            "Alternar entre tradicional e innovador",
            "Progresar de simple a complejo",
            "Adaptar según respuesta del atleta"
        ]
    },
    
    "combinacion_inteligente": {
        "concepto": "Fusionar ejercicios de manera lógica",
        "criterios": [
            "Compatibilidad biomecánica",
            "Progresión lógica de dificultad",
            "Transferencia al deporte",
            "Factibilidad práctica"
        ]
    },
    
    "individualizacion": {
        "concepto": "Personalizar según el atleta específico",
        "factores": [
            "Historial de lesiones",
            "Preferencias personales",
            "Respuesta al entrenamiento",
            "Contexto situacional"
        ]
    }
}

# ================================
# 🏃‍♂️ RUTINAS ESPECÍFICAS POR DEPORTE
# ================================

SPORT_SPECIFIC_ROUTINES = {
    "futbol": {
        "activacion": [
            "Foam rolling + movilidad articular específica",
            "Activación glútea x3 + core dinámico",
            "Movimientos balísticos progresivos",
            "Cambios de dirección a baja intensidad"
        ],
        "potencia": [
            "Salidas explosivas desde diferentes posiciones",
            "Sprints multidireccionales 15-20m",
            "Saltos reactivos + aterrizaje controlado",
            "Trabajo pliométrico lateral"
        ],
        "especifico": [
            "Situaciones 1v1 técnico-tácticas",
            "Remates con salto bajo fatiga",
            "Control + pase bajo presión temporal",
            "Jugadas aéreas específicas"
        ],
        "metabolico": [
            "RSA fútbol: 6x30m (15'' trabajo / 45'' pausa)",
            "Circuito técnico-físico específico",
            "Intervalos variables (alta-media intensidad)",
            "Finisher: sprints con cambio de dirección"
        ]
    },
    
    "rugby": {
        "activacion": [
            "Movilidad dinámica completa",
            "Contactos progresivos controlados", 
            "Aceleraciones cortas específicas",
            "Activación sistema neuromuscular"
        ],
        "potencia": [
            "Empujes explosivos (scrum simulation)",
            "Saltos pliométricos multidireccionales",
            "Sprints con resistencia (sled/paracaídas)",
            "Trabajo de contacto explosivo"
        ],
        "especifico": [
            "Técnica de tackleo progresivo",
            "Situaciones de ruck específicas",
            "Line-out jumps con precisión",
            "Manejo de balón bajo contacto"
        ],
        "metabolico": [
            "RSA rugby: 8''x24'' x (2-3') pausa",
            "Circuitos de contacto + sprint",
            "Intervalos específicos del juego",
            "Trabajo de resistencia explosiva"
        ]
    },
    
    "tenis": {
        "activacion": [
            "Sombras técnicas dinámicas",
            "Desplazamientos laterales específicos",
            "Rotaciones dinámicas de tronco",
            "Activación cadena cinética del golpe"
        ],
        "potencia": [
            "Golpes explosivos con implementos",
            "Saltos reactivos multidireccionales",
            "Cambios de dirección específicos",
            "Trabajo pliométrico de tren superior"
        ],
        "especifico": [
            "Voleas bajo presión temporal",
            "Servicios técnicos con potencia",
            "Recuperaciones rápidas en cancha",
            "Situaciones de punto específicas"
        ],
        "metabolico": [
            "Intervalos tipo set de tenis",
            "Circuitos técnico-físicos",
            "RSA específico de tenis (15-45seg)",
            "Trabajo de resistencia a la velocidad"
        ]
    },
    
    "natacion": {
        "activacion": [
            "Movilidad específica de hombros",
            "Activación escapular progresiva",
            "Coordinación brazada en seco",
            "Preparación sistema respiratorio"
        ],
        "potencia": [
            "Pull-ups explosivos específicos",
            "Empujes acuáticos simulados",
            "Patadas potentes fuera del agua",
            "Trabajo de rotación de tronco"
        ],
        "especifico": [
            "Técnica respiración específica",
            "Virajes explosivos simulados",
            "Salidas desde plataforma",
            "Brazadas específicas por estilo"
        ],
        "metabolico": [
            "Intervalos específicos por distancia",
            "Sets de velocidad progresiva",
            "Trabajo de umbral específico",
            "Recuperación activa dirigida"
        ]
    },
    
    "atletismo": {
        "activacion": [
            "Educativos de carrera específicos",
            "Activación neuromuscular progresiva",
            "Movilidad dinámica de caderas",
            "Preparación biomecánica específica"
        ],
        "potencia": [
            "Aceleraciones progresivas específicas",
            "Saltos técnicos por especialidad",
            "Ejercicios de frecuencia de paso",
            "Trabajo de amplitud específica"
        ],
        "especifico": [
            "Técnica específica por prueba",
            "Simulación de competencia",
            "Trabajo de ritmo específico",
            "Transferencia técnica directa"
        ],
        "metabolico": [
            "Intervalos específicos por distancia",
            "Fartlek específico del evento",
            "Tempo runs controlados",
            "Recuperación metabólica dirigida"
        ]
    }
}

# ================================
# 🎯 MÉTODOS DE ENTRENAMIENTO ESPECÍFICOS
# ================================

TRAINING_METHODS = {
    "rsa_protocols": {
        "definicion": "Repeated Sprint Ability - Capacidad de repetir sprints",
        "protocolos": [
            "Fútbol: 6x30m (15'' trabajo / 45'' pausa) x 3 series",
            "Rugby: 8''x24'' x (2-3') pausa entre series",
            "Básquet: 6x15m (10'' trabajo / 50'' pausa) x 4 series",
            "Tenis: 15''x45'' modalidad específica del deporte"
        ]
    },
    
    "circuitos_metabolicos": {
        "definicion": "Circuitos de alta intensidad específicos",
        "formatos": [
            "EMOM (Every Minute On the Minute): 1 ejercicio/min",
            "Tabata modificado: 20''ON/10''OFF x 8 rounds",
            "Circuito 5x series: 30-45seg trabajo/15-30seg pausa",
            "Escalera ascendente: 1-2-3-4-5 repeticiones"
        ]
    },
    
    "complejos_franceses": {
        "definicion": "Fuerza + Potencia + Velocidad en secuencia",
        "ejemplos": [
            "Sentadilla pesada + Salto al cajón + Sprint 20m",
            "Press banca + Flexiones explosivas + Lanzamiento balón",
            "Peso muerto + Saltos reactivos + Aceleración",
            "Dominadas + Lanzamientos + Movimientos específicos"
        ]
    },
    
    "entrenamiento_contraste": {
        "definicion": "Alternancia entre cargas pesadas y explosivas",
        "protocolos": [
            "Carga pesada (85-95%) + Ejercicio explosivo",
            "3-5 min pausa entre métodos",
            "4-6 series por complejo",
            "Aplicación específica al gesto deportivo"
        ]
    }
}

# ================================
# 🏋️‍♂️ IMPLEMENTOS Y EQUIPAMIENTO
# ================================

EQUIPMENT_VARIATIONS = {
    "sin_equipamiento": [
        "Movimientos animales complejos",
        "Ejercicios isométricos progresivos",
        "Pliometría corporal avanzada",
        "Patrones de movimiento funcional",
        "Trabajo de movilidad dinámica"
    ],
    
    "equipamiento_basico": [
        "Bandas elásticas multifuncionales",
        "Balones medicinales variables",
        "TRX o sistemas de suspensión",
        "Conos y escaleras de agilidad",
        "Foam roller y pelotas lacrosse"
    ],
    
    "equipamiento_avanzado": [
        "Kettlebells de diferentes pesos",
        "Barras olímpicas y discos",
        "Cajones pliométricos variables",
        "Sleds y paracaídas de resistencia",
        "Sistemas de poleas multifuncionales"
    ],
    
    "implementos_especificos": [
        "Implementos específicos del deporte",
        "Balones por especialidad deportiva",
        "Raquetas/palos según deporte",
        "Equipamiento técnico específico",
        "Simuladores de gestos deportivos"
    ]
}

# ================================
# 📊 NIVELES DE PROGRESIÓN
# ================================

PROGRESSION_LEVELS = {
    "principiante": {
        "enfoque": "Aprendizaje de patrones básicos + acondicionamiento general",
        "volumen": "2-3 entrenamientos/semana",
        "intensidad": "60-75% de capacidad máxima",
        "complejidad": "Ejercicios básicos con supervisión"
    },
    
    "intermedio": {
        "enfoque": "Desarrollo de capacidades específicas + introducción técnica",
        "volumen": "3-4 entrenamientos/semana",
        "intensidad": "70-85% de capacidad máxima", 
        "complejidad": "Ejercicios combinados + elementos específicos"
    },
    
    "avanzado": {
        "enfoque": "Optimización del rendimiento + especificidad alta",
        "volumen": "4-5 entrenamientos/semana",
        "intensidad": "80-95% de capacidad máxima",
        "complejidad": "Ejercicios complejos + transferencia directa"
    },
    
    "semi_profesional": {
        "enfoque": "Periodización específica + picos de rendimiento",
        "volumen": "5-6 entrenamientos/semana",
        "intensidad": "85-100% de capacidad máxima",
        "complejidad": "Entrenamientos específicos + competencia"
    },
    
    "elite": {
        "enfoque": "Rendimiento máximo + individualización total",
        "volumen": "6+ entrenamientos/semana",
        "intensidad": "Variable según periodización",
        "complejidad": "Completamente específico + innovación constante"
    }
}

# ================================
# 🎨 GENERADOR DE RUTINAS CREATIVAS
# ================================

TRAINING_METHODS = {
    "principiante": {
        "enfoque": "Técnica y estabilidad",
        "metodos": ["circuitos básicos", "progresiones controladas", "tiempo bajo tensión"],
        "intensidad": "60-70% RPE 6-7"
    },
    "intermedio": {
        "enfoque": "Progresión y variabilidad", 
        "metodos": ["complejos de fuerza", "contrastes", "periodización ondulatoria"],
        "intensidad": "70-85% RPE 7-8"
    },
    "avanzado": {
        "enfoque": "Especificidad y rendimiento máximo",
        "metodos": ["CAT", "clusters", "francés", "búlgaro", "westside"],
        "intensidad": "85-95% RPE 8-9"
    },
    "elite": {
        "enfoque": "Periodización compleja y transferencia",
        "metodos": ["VBT", "periodización conjugada", "potenciación post-activación"],
        "intensidad": "Variable según fase"
    }
}

CREATIVE_COMBINATIONS = [
    "Ejercicio + salto reactivo",
    "Movimiento unilateral + rotación",
    "Fuerza + componente cognitivo",
    "Isométrico + explosivo",
    "Cadena abierta + cerrada",
    "Plano sagital + frontal + transversal",
    "Equipamiento combinado",
    "Tempo variable + pausa"
]

# ================================
# 🎨 FUNCIONES DE GENERACIÓN INTELIGENTE
# ================================

def generate_hybrid_routine(sport, level, equipment="basico", focus="integral"):
    """
    Genera una rutina híbrida basada en principios, no en listas fijas
    Combina ejercicios tradicionales con innovaciones funcionales
    
    Args:
        sport: Deporte específico
        level: Nivel del atleta
        equipment: Equipamiento disponible
        focus: Enfoque del entrenamiento
    
    Returns:
        Dict con metodología estructurada (sin ejercicios específicos)
    """
    
    # Obtener principios según el deporte
    sport_principles = get_sport_adaptation_principles(sport)
    level_guidelines = get_progression_guidelines(level)
    
    routine_framework = {
        "info": {
            "deporte": sport,
            "nivel": level,
            "enfoque_metodologico": focus,
            "duracion_total": "90-120 minutos"
        },
        "metodologia": {},
        "principios_aplicados": sport_principles,
        "guias_nivel": level_guidelines
    }
    
    # Generar framework metodológico por bloques
    for bloque_key, bloque_info in ROUTINE_METHODOLOGY.items():
        routine_framework["metodologia"][bloque_key] = {
            "nombre": bloque_info["nombre"],
            "duracion": bloque_info["duracion"],
            "principios": bloque_info["principios"],
            "categorias_sugeridas": bloque_info["categorias"],
            "adaptacion_nivel": level_guidelines["parametros"],
            "enfoque_deporte": sport_principles["enfoque"][:2]  # Primeros 2 enfoques
        }
    
    return routine_framework

def get_sport_adaptation_principles(sport):
    """Obtiene principios de adaptación según el tipo de deporte"""
    sport_lower = sport.lower()
    
    # Clasificar deporte por tipo
    for category, data in SPORT_ADAPTATION_PRINCIPLES.items():
        for example_sport in data["ejemplos"]:
            if example_sport in sport_lower:
                return data
    
    # Default para deportes no clasificados
    return SPORT_ADAPTATION_PRINCIPLES["deportes_intermitentes"]

def get_progression_guidelines(level):
    """Obtiene guías de progresión según el nivel"""
    level_lower = level.lower().replace(" ", "_")
    return PROGRESSION_GUIDELINES.get(level_lower, PROGRESSION_GUIDELINES["intermedio"])

def get_creativity_strategies():
    """Obtiene estrategias de creatividad para variar entrenamientos"""
    return CREATIVITY_PRINCIPLES

def get_exercise_categories():
    """Obtiene categorías de ejercicios para guiar la selección"""
    return EXERCISE_CATEGORIES

def generate_variation_suggestions(base_category, level):
    """
    Genera sugerencias de variación sin dar ejercicios específicos
    
    Args:
        base_category: Categoría base del ejercicio
        level: Nivel del atleta
    
    Returns:
        Dict con principios de variación
    """
    
    guidelines = get_progression_guidelines(level)
    
    suggestions = {
        "categoria_base": base_category,
        "nivel": level,
        "principios_variacion": CREATIVITY_PRINCIPLES["variacion_sistematica"]["estrategias"],
        "metodos_preferidos": guidelines["metodos_preferidos"],
        "parametros_recomendados": guidelines["parametros"],
        "filosofia": guidelines["filosofia"]
    }
    
    return suggestions

def customize_methodology_for_athlete(base_methodology, athlete_info):
    """
    Personaliza la metodología según información específica del atleta
    
    Args:
        base_methodology: Metodología base generada
        athlete_info: Información del atleta
    
    Returns:
        Metodología personalizada
    """
    customized = base_methodology.copy()
    
    # Personalización según objetivos
    if athlete_info.get("objetivos"):
        customized["personalizacion"] = {
            "objetivos_especificos": athlete_info["objetivos"],
            "adaptaciones_sugeridas": "Enfatizar bloques relacionados con objetivos",
            "progresion": "Ajustar intensidad según respuesta individual"
        }
    
    # Consideraciones especiales
    if athlete_info.get("limitaciones"):
        customized["consideraciones_especiales"] = {
            "limitaciones": athlete_info["limitaciones"],
            "adaptaciones": "Modificar ejercicios según limitaciones",
            "alternativas": "Usar categorías compatibles"
        }
    
    return customized

# ================================
# 🔧 FUNCIONES DE COMPATIBILIDAD
# ================================

def get_training_method_examples(method_type="hybrid_approach"):
    """Obtiene ejemplos de métodos de entrenamiento"""
    if method_type in TRAINING_METHODS:
        return TRAINING_METHODS[method_type]
    return {"enfoque": "Enfoque híbrido inteligente", "metodos": ["variabilidad_sistematica"]}

def get_equipment_guidelines(equipment_type="basico"):
    """Obtiene guías para diferentes tipos de equipamiento"""
    equipment_guidelines = {
        "sin_equipamiento": {
            "enfoque": "Maximizar ejercicios de peso corporal",
            "categorias_preferidas": ["patrones_fundamentales", "movimientos_funcionales"]
        },
        "basico": {
            "enfoque": "Combinar peso corporal con implementos simples",
            "categorias_preferidas": ["hibrido_tradicional_funcional"]
        },
        "avanzado": {
            "enfoque": "Utilizar toda la gama de equipamiento disponible",
            "categorias_preferidas": ["metodos_avanzados", "especificos_deporte"]
        }
    }
    
    return equipment_guidelines.get(equipment_type, equipment_guidelines["basico"])

def generate_weekly_methodology(sport, level, sessions_per_week=4):
    """
    Genera metodología semanal basada en principios
    
    Args:
        sport: Deporte específico
        level: Nivel del atleta
        sessions_per_week: Número de sesiones por semana
    
    Returns:
        Dict con estructura metodológica semanal
    """
    level_data = get_progression_guidelines(level)
    sport_data = get_sport_adaptation_principles(sport)
    
    weekly_methodology = {
        "sesiones_semanales": sessions_per_week,
        "filosofia_nivel": level_data["filosofia"],
        "enfoque_deporte": sport_data["enfoque"][0],
        "distribucion_metodologica": {}
    }
    
    # Distribución metodológica según número de sesiones
    if sessions_per_week <= 2:
        weekly_methodology["distribucion_metodologica"] = {
            "Sesión 1": {
                "enfoque": "Desarrollo integral",
                "bloques": ["activacion", "principal_fuerza", "integracion"],
                "filosofia": "Combinar tradicional con funcional"
            },
            "Sesión 2": {
                "enfoque": "Acondicionamiento específico",
                "bloques": ["activacion", "principal_metabolico", "especifico"],
                "filosofia": "Enfasis en capacidades específicas del deporte"
            }
        }
    elif sessions_per_week == 3:
        weekly_methodology["distribucion_metodologica"] = {
            "Sesión 1": {
                "enfoque": "Fuerza y potencia",
                "bloques": ["activacion", "principal_fuerza", "finalizacion"],
                "filosofia": "Base tradicional con elementos funcionales"
            },
            "Sesión 2": {
                "enfoque": "Específico del deporte",
                "bloques": ["activacion_especifica", "integracion", "aplicacion"],
                "filosofia": "Transferencia directa al rendimiento"
            },
            "Sesión 3": {
                "enfoque": "Acondicionamiento y prevención",
                "bloques": ["movilidad", "metabolico", "prevencion"],
                "filosofia": "Mantenimiento y regeneración"
            }
        }
    else:  # 4+ sesiones
        weekly_methodology["distribucion_metodologica"] = {
            "Principio_general": "Distribución equilibrada entre todos los métodos",
            "Variabilidad": "Rotar entre enfoques tradicionales e innovadores",
            "Progresion": "Aumentar complejidad progresivamente",
            "Individualizacion": "Adaptar según respuesta del atleta"
        }
    
    return weekly_methodology
